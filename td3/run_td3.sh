...
tmux has-session -t "td3" 2>/dev/null
if [ $? == 0 ]; then
    tmux kill-session -t "td3"
fi
...
for ((i=0; i<3; i++)); do
    ...
    for ((j=0; j<envs_per_window; j++)); do
        ...
        for ((k=start; k<=end; k++)); do
            ...
            tmux send-keys "conda activate rltd3" C-m
            tmux send-keys "echo 'Running ${environments[$k]}'" C-m

            for seed in "${seeds[@]}"; do
                tmux send-keys "echo 'Seed $seed'; python main.py --env=${environments[$k]} --seed $seed" C-m
            done
        done
    done
done

tmux attach-session -t "td3"
