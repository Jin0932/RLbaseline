...
def run_ddpg(args, env_id,
    ...
    wandb.init(
            ...
            project="CompareAlgo",
            entity="lzudslab",
            group=f"{env_id}-{args.algo}-seed={seed}",
            job_type = f"seed={seed}", 
            name=f"{env_id}-{args.algo}-seed={seed}"
    )
    args = locals()
    ...
    for t in range(max_iter + 1):
        ...
        state = next_state
        ep_reward += reward
        wandb.log({"ep_reward":ep_reward, "step":t})
        ...
        if t % eval_interval == 0:
            eval_avg_reward = agent.eval(env_id, t)
            eval_logger.writerow(eval_avg_reward)
            wandb.log({"eval_reward":eval_avg_reward[1], "step":eval_avg_reward[0]})
